function [decoder_output] = FWC_COV_Dencoder(G,k,channel_output)
...
% % function [decoder_output,survivor_state,comulated_metric] = COV_Dencoder(G,k0,channel_out);
...
% 
% k               =   k0;
% channel_output  =   channel_out;
% 
% n               =   size(G,1); % fin so hang MT
% 
% % Check the size.
% if rem(size(G,2),k)~=0
%     error('kich thuoc G va k khong phu hop');
% end
% if rem(size(channel_output,2),n)~=0
%     error('kich thuoc dau ra kenh khong dung ');
% end
% L                   =   size(G,2)/k;
% number_of_states    =   2^((L-1)*k); %=4
% 
% % tao ma tran chuyen dich trang thai, matran dau ra va ma tran dau vao
% for j=0:number_of_states-1
%     for l = 0:2^k-1
%         [next_state, memory_contents]   =   nxt_stat(j,l,L,k);
%         input(j+1,next_state +1)        =   l;
...
% 
% %%%%%%%%%%%%%%%%%%%%%%%
% state_metric            =   zeros(number_of_states,2);                      % tao Ma tran [0] gom 4 hangx2cot
% depth_of_trellis        =   length(channel_output)/n;                       % do sau = 7
% channel_output_matrix   =   reshape(channel_output,n,depth_of_trellis);     % tao ma tran dau ra tu channelout 2 hang 7 cot
% survivor_state=zeros(number_of_states,depth_of_trellis +1);                 % tao MT 0 4x8
% 
% % bat dau giai ma cac dau ra kenh khong duoi
% for i=1:depth_of_trellis - L +1
%     flag        = zeros(1,number_of_states);
%     if i <= L
%         step    = 2^((L-i)*k);
...
% end
% 
% % start decoding of the tail channel output
% for i=depth_of_trellis-L+2:depth_of_trellis
%     flag        = zeros(1,number_of_states);
%     last_stop   = number_of_states/(2^((i-depth_of_trellis + L -2)*k));
%     for j=0:last_stop-1
%         branch_metric   = 0;
%         branch_output   = deci2bin(output(j+1,1),n);
%         for ll=1:n
%             branch_metric = branch_metric + metric(channel_output_matrix(ll,i),binary_output(ll));
%         end
...
% end
% 
% %Tao dau ra bo gia ma tu duong dan toi uu (optimum path)
% state_sequence                      = zeros(1,depth_of_trellis + 1);
% state_sequence(1,depth_of_trellis)  = survivor_state(1,depth_of_trellis+1);
% for i=1:depth_of_trellis
