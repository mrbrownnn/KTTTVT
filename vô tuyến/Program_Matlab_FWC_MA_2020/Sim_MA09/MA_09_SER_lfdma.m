function SER = SER_lfdma(SNR,numRun,inputBlockSize,FFTsize,dataType,...
                         CPsize,subband,channelType,equalizerType)
%--------------------------------------------------------------------------
% Tinh toan ty so loi ky hieu (SER) trong SC-FDMA che do tap trung LFDMA
%--------------------------------------------------------------------------
...
for n = 1:length(SNR),
    ...
    for k = 1:numRun,
        ...
        % Sap xep song mang con LFDMA
        inputSamples([1:inputBlockSize]+...
        ...
        Y = Y([1:inputBlockSize]+inputBlockSize*subband);
        % Tim dap ung kenh cho cac song mang con tap trung
        H_eff = H_channel([1:inputBlockSize]+inputBlockSize*subband);
        % Can bang kenh
        if channelType~='0000'
