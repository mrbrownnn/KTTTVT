...
function p = MA_04_prob_error(SNRbdB,signaling,b,opt1,opt2)

%--- Finds the symbol/bit error probability for given SNRbdB (Table 7.1)-

if nargin<5, 
    opt2 = 'coherent'; 
end  % opt2='coherent' or 'noncoherent'
...
if signaling(1:3)=='ASK' % ASK (PAM)
    if lower(opt2(1))=='c' % ASK coherent --> Eq.(7.1.5)
        for i=1:NSNR, 
            p(i) = 2*(M-1)/M*Q(sqrt(3*b*SNRb(i)/(M^2-1))); 
        end
        if lower(opt1(1))=='b', 
            p   = p/b; 
        end
...
elseif signaling(1:3)=='FSK'
    ...
    if lower(opt1(1))=='b'&b>1,
        p = p*tmp; 
    end
elseif signaling(1:3)=='PSK'  % Eq.(7.3.7)
    for i=1:NSNR, 
        p(i)=(1+(b>1))*MA_04_Qfun(sqrt(b*SNRb(i))*sin(pi/M));
    end
    if lower(opt1(1))=='b'&b>1,
        p = p/b;
    end
elseif signaling(1:3)=='DPS' % DPSK
    if b==1  % Eq.(7.4.8)
        for i=1:NSNR, 
            p(i) = 2*MA_04_Qfun(sqrt(b*SNRb(i)/2)*sin(pi/M));
        end
    else
        for i=1:NSNR, 
            p(i) = exp(-SNRb(i)/2)/2; 
