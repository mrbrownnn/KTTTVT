function [p, pye, b] = NVD15_bwa_test(iterations,states,out);
% File: the same NVD15_bwa.m

len     = length(out);
p       = [0.9 0.05 0.05; 0.1 0.8 0.1; 0.1 0.2 0.7];
pye     = [0.3 0.3 0.4];
b       = [0.9 0.8 0.7; 0.1 0.2 0.3];

% p       = input('Nhap ma tran chuyen dich trang thai khoi tao P = ');
% pye     = input('Nhap vector xac suat trang thai khoi tao pye = ');
% b       = input('Nhap ma tran xac suat ky hieu ra khoi tao B = ') ;

alpha   =zeros(len,states);
beta    =zeros(len,states);
eta     =zeros(states,states);
gamma   =zeros(1,states);
scale   =zeros(len,1);
...
if iplot==1
   ...
   xlabel('LÆp','fontname','.Vntime','color','b','fontsize',12);
   ylabel('Log likelihood','fontname','.Vntime','color','b','fontsize',12);
end