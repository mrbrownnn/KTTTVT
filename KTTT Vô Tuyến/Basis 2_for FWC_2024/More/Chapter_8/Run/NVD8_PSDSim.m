...
h89 = figure
set(h89,'color','c');
set(h89,'name','H8.9: NVD');

subplot(2,1,1)
plot(f(1:5000),psd1(1:5000))
grid; axis([0 100 -70 10]);
xlabel('TÇn sè, Hz','fontname','.Vntime','fontsize',12);
ylabel('PSD','fontname','.Vntime','fontsize',14);
title('¦íc tÝnh mËt ®é phæ c«ng suÊt PSD: Kh«ng ®­îc lÊy trung b×nh',...
...
xlabel('TÇn sè, Hz','fontname','.Vntime','fontsize',12);
ylabel('PSD','fontname','.Vntime','fontsize',14);
title('¦íc tÝnh mËt ®é phæ c«ng suÊt PSD: §­îc lÊy trung b×nh',...
    'fontname','.Vntime','fontsize',14);