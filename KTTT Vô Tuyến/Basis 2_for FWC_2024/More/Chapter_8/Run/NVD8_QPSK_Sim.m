...
T_bit           = 0.5;      % Bit time
Rb              = 1/T_bit;
fc              = 4/T_bit;  % input('Nhap tan so song mang Hz (4) =   ');
...
elseif I_plot ==3|I_plot==4|I_plot==5 % Bieu do mat, Duong bao, Bieu do pha, 
    ...
    if I_filt ==1        
        BW      = 0.5;       %input('    Nhap do rong bang thong bo loc: B= [0 den 0.8]R_b: ');
        n_order = 4;    %input('   Nhap bac bo loc (1,2,3,4) =  ');
    ...
    if I_plot==3  % Bieu do mat====        
        ...
        for n=2:fix((N_sig-5*samp_bit)/N_sym)
            ...
            elseif I_filt ==2
                title(['BiÓu ®å m¾t tÝn hiÖu QPSK khong ®­îc läc '],'fontname','.Vntime','fontsize',12);
            end
            if n==2
                hold on
            end
...
figure(2);
    ...
    title(['Phæ tÝn hiÖu QPSK; TÇn sè trung t©m = ',num2str(fc),...
        ...
        'fontname','.Vntime','fontsize',12);
    grid on   
    L   = legend('PSD cña QPSK: M« pháng','PSD cña QPSK: TÝnh to¸n ');
    set(L, 'fontname','.Vntime','fontsize',13);        
end